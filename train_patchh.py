...
def conf():
    ...
    datasetFtrain = CelebADataset("/net/girlschool/besnier/CelebA_dataset/multi_dataset/img_F_train",
                             f,
                             transforms.Compose([transforms.Resize(64),
                                                 transforms.ToTensor(),
                                                 transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ...
    trainloaderF = torch.utils.data.DataLoader(datasetFtrain, batch_size=64, shuffle=True, num_workers=1, drop_last=True)

    datasetFtest = CelebADataset("/net/girlschool/besnier/CelebA_dataset/multi_dataset/img_F_test",
                             f,
                             transforms.Compose([transforms.Resize(64),
                                                 transforms.ToTensor(),
                                                 transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
    ...
    testloaderF = torch.utils.data.DataLoader(datasetFtest, batch_size=64, shuffle=False, num_workers=1, drop_last=True)


@ex.main
def main(netG, netDlow, netDhigh, f_bruit, epoch, device, param, cuda, dataloaderH, trainloaderF, testloaderF, optimizerG, optimizerDlow, optimizerDhigh):
    file = 'AmbientGAN/train_patch/hf_1_1_100_1'
    print(file)
    ...
    for _ in bar_epoch:
        for i, (xf, xbf), (xh, xbh) in zip_longest(tqdm(range(len(trainloaderF))), trainloaderF, dataloaderH, fillvalue=(None, None)):
            ...
            (losshigh+losslow+100*losscomp+lossmodule).backward()
            optimizerG.step()
            ...
            #####################
            if i % 250 == 0 and i > 1:
                testbar = tqdm(range(len(testloaderF)))
                mse_test = []
                ref = []
                for j, (xhqt, xlqt) in zip(testbar, testloaderF):
                    if j > 100:
                        break
                    if turn:
                        save_xb = xlqt
