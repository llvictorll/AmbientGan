...
from dataset import CrowdDatasetspe
...
def conf():
    ...
    epoch = 100
    file = 'SRGAN/Crow_AGAN_face_replace'
    f = F_bruit(0.9)

    traindataset = CrowdDatasetspe("/net/girlschool/besnier/Crow/train",
    ...
    testdataset = CrowdDatasetspe("/net/girlschool/besnier/Crow/test",
...
@ex.automain
def main(netG, netD, epoch, trainloader, testloader, optimizerG, optimizerD, file, device, f):
    print(file)
    netG.train()
    netD.train()
    ...
    for e in bar_epoch:
        for i, (x, xb, v, vb, (a,b,c,d)) in zip(tqdm(range(len(trainloader))), trainloader):
            ...
            if i % 250 == 0:
                ...
                for j, (x, xb, v, vb, (a,b,c,d)) in zip(testbar, testloader):
                    ...
                    if turn < 10:
                        if e == 0 and i == 0:
                            ...
                            print_img(xb, 'image_bruit', file+'/'+str(turn))
                        copy_xb = xb
                ...
                turn = 0
                sauvegarde(file, np.array(mse_test).mean())
